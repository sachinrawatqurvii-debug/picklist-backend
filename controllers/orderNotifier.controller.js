...
const generateAlterationPDF = async (upRecords = [], downRecords = [], cuttingRecords = [], expectedQtyMap = {}) => {
    return new Promise((resolve, reject) => {
        try {
            const doc = new PDFDocument({ margin: 50 });
            const buffers = [];
          ...
          .text(`Alteration and Cutting Records: Id -(${cuttingRecords[0]?.picklist_id || 0})`, 50, 50);
            ...
            // Cutting Records Section - NEW SECTION
            if (cuttingRecords.length > 0) {
                doc.fillColor('#e74c3c')
                    .fontSize(14)
                    .font('Helvetica-Bold')
                    .text('Cutting Records (Found but Gone Cutting)', 50, yPosition);

                yPosition += 30;

                // Table headers for cutting records
                // const cuttingHeaders = ['Style No', 'Size', 'To Size', 'Rack Space', 'Status', 'Available Stock', 'Rating'];
                const cuttingHeaders = ['Style No', 'Size', 'To Size', 'Rack Space', 'Status', 'Available Stock',];
                drawTableHeader(doc, cuttingHeaders, yPosition);
                yPosition += 25;

                // Table rows for cutting records
                cuttingRecords.forEach((record, index) => {
                    if (yPosition > 700) {
                        doc.addPage();
                        yPosition = 50;
                    }
            ...
            if (upRecords.length > 0) {
                ...
                // Table headers for UP records
                const upHeaders = ['Style No', 'From Size', 'To Size', 'Rack Space', 'Status', 'Available Stock'];
                drawTableHeader(doc, upHeaders, yPosition);
                yPosition += 25;
            ...
            if (downRecords.length > 0) {
                ...
                // Table headers for DOWN records
                const downHeaders = ['Style No', 'From Size', 'To Size', 'Rack Space', 'Status', 'Available Stock'];
                drawTableHeader(doc, downHeaders, yPosition);
                yPosition += 25;
...
const sendNotificationToEmail = async (req, res, next) => {
    ...
    try {
        ...
        try {
            ...
            const pdfBuffer = await generateAlterationPDF(
                status.aleration_For_Up_Size?.records || [],
                status.aleration_For_Down_Size?.records || [],
                status.actualSizeFound?.Expected_For_Found_But_Gone_Cutting_Qty_Record || [],
                expectedQtyMap
            );

            pdfAttachment = {
                filename: `not-found-records-and-alteration-records-${new Date().toISOString().split('T')[0]}.pdf`,
                content: pdfBuffer,
                contentType: 'application/pdf'
            };
        } catch (pdfError) {
                        ...
                        <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(200px, 1fr)); gap:15px;">
                         <div>
                            ...
                            <div>
                             ...
                             <div>
                                <div style="font-size:16px; opacity:0.9;">Total Found : ${status.totalFound || 0} </div>
                            </div>
                             <div>
                                <div style="font-size:16px; opacity:0.9;">Total Alter : ${status.totalAlter || 0} </div>
                            </div>
                            <div>
                                <div style="font-size:16px; opacity:0.9;">Actual Size Found : ${status.actualSizeFound?.Actual_Found_Qty || 0}</div>
                            </div>
                             <div>
                                <div style="font-size:16px; opacity:0.9;">Total Cutting : ${(status.totalOrders || 0) - (status.totalFound || 0)}</div>
                            </div>
  ...
  Efficiency : ${calculatePercentage(status.actualSizeFound?.Expected_To_Found_Actual_Size_Qty || 0, status.actualSizeFound?.Actual_Found_Qty || 0) + '%'}</div>
        ...
        // const recipients = [EMAIL_ADDRESS];
        const recipients = [ "logistics@qurvii.com", "anu@qurvii.com", "kajal@qurvii.com"];
